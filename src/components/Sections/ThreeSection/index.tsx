...
function Card({ image, title, description, link, color }: CardProps) {
  return (
    <div className="mx-auto flex max-w-3xs flex-col gap-5 bg-[#E9F5F3] px-8 py-5 [box-shadow:2px_4px_30.1px_5px_rgba(0,0,0,0.07)]">
      <div
      ...
      <strong className={`${poppins.className} text-sm/[1.1] font-bold`}>
        {title}
      </strong>
      <p
        className={`${roboto.className} mb-5 text-left text-xs/[1.3] text-[#525252]`}
      >
        {description}
      </p>
      <a
        href=""
        className={`${poppins.className} text-principal text-[0.625rem]/[1.1] font-bold`}
      >
        {link}
      </a>
    </div>
...
export function ThreeSection() {
  return (
    <section className="relative min-h-[calc(100dvh-80px)] w-full px-10 pt-20 md:px-0">
      ...
      <div className="mx-auto w-4/5">
        <h2
          className={`${ptSerif.className} text-[#0A0A22} mb-1.5 text-left text-2xl/[1.1] font-bold`}
        >
          Comece assim
        </h2>
        <p
        ...
        <article className="mt-5 flex flex-wrap gap-y-20">
          <Card
            image={meta}
            title="Meditação"
